...
const uploadBackground = async (file) => {
  ...
  try {
    ...
    const { error: upErr } = await supabase.storage.from("love-media").upload(path, file, { upsert: false });
    if (upErr) return alert(upErr.message);
    const { data: publicData } = supabase.storage.from("love-media").getPublicUrl(path);
    const { error } = await supabase.from("photos").insert({
      owner_id: session.user.id, image_url: publicData.publicUrl, caption, photo_date
    });
    if (error) alert(error.message); else { setModal(null); await refresh(); }
  };
